...
import { chatMessages, users } from "@shared/schema.js";
...
router.get("/chat/:userId", async (req, res) => {
    ...
    try {
        ...
        const messages = await db
            ...
            .from(chatMessages)
            .where(or(and(eq(chatMessages.senderId, currentUserId), eq(chatMessages.receiverId, otherUserId)), and(eq(chatMessages.senderId, otherUserId), eq(chatMessages.receiverId, currentUserId))))
...
router.get("/chat", async (req, res) => {
    ...
    try {
        ...
        const conversations = await db
            ...
            .from(users)
            .innerJoin(chatMessages, or(and(eq(chatMessages.senderId, users.id), eq(chatMessages.receiverId, currentUserId)), and(eq(chatMessages.receiverId, users.id), eq(chatMessages.senderId, currentUserId))))
            .groupBy(users.id);
...
router.patch("/chat/:userId/read", async (req, res) => {
    ...
    try {
        ...
        const result = await db
            .update(chatMessages)
            .set({ read: true })
            .where(and(eq(chatMessages.senderId, otherUserId), eq(chatMessages.receiverId, currentUserId), eq(chatMessages.read, false)));
