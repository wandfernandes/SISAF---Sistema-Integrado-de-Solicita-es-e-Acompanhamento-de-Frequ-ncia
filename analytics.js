...
export async function getRequestAnalytics(userId) {
...
export async function getMedicalLeaveAnalytics(userId) {
    const whereClause = userId ? eq(medicalLeaves.userId, userId) : undefined;
    // Obter estatísticas gerais
    const statsResult = await db
        .select({
        totalLeaves: sql `count(*)`,
        averageProcessingTime: sql `
        avg(case 
          when status != 'pending' 
    ...
    const leavesByWorkLocation = await db
        ...
        count: sql `count(distinct ${medicalLeaves.userId})`,
    })
        .from(medicalLeaves)
...
export async function getVacationAnalytics(userId) {
    const whereClause = userId ? eq(vacationPeriods.userId, userId) : undefined;
    const today = new Date().toISOString();
    // Obter estatísticas gerais
    const statsResult = await db
        .select({
        totalVacations: sql `count(*)`,
        currentlyOnVacation: sql `
        count(case 
    ...
    const vacationsByMonth = await db
        ...
        .from(vacationPeriods)
        .where(and(sql `start_date is not null`, whereClause))
    ...
    const vacationsByWorkLocation = await db
        .select({
        location: users.workLocation,
        count: sql `count(distinct ${vacationPeriods.userId})`,
        // Adicionando nomes de usuários por localização
        userNames: sql `array_agg(distinct ${users.fullName})`,
    })
        .from(vacationPeriods)
    ...
    const criticalPeriods = await db
        .select({
        startDate: sql `to_char(start_date, 'YYYY-MM-DD')`,
        endDate: sql `to_char(end_date, 'YYYY-MM-DD')`,
        count: sql `count(*)`,
        // Adicionando nomes de usuários para períodos críticos
        userNames: sql `array_agg(${users.fullName})`,
    })
        .from(vacationPeriods)
        .leftJoin(users, eq(vacationPeriods.userId, users.id))
        .where(and(sql `status = 'approved'`, sql `start_date >= ${today}::date`, whereClause))
...
export async function getManagementAnalytics() {
    ...
    const absenteeismStats = await db
        .select({
        unit: users.workUnit,
        totalEmployees: sql `count(distinct users.id)`,
        absences: sql `
